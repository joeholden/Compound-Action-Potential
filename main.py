...
def main(sweep_number):
    ...
    def find_impulse(cap_file):
        ...
        cap_file.setSweep(sweepNumber=sweep_number, channel=1)
    ...
    def find_baseline_noise(cap_file):
        ...
        cap_file.setSweep(sweepNumber=sweep_number, channel=0)
    ...
    # If the impulse spike doesnt cross below the zero line, it will be lumped in with the cap peak as a single area
    # Simply re-setting the CAP peak to start after index 16410 fixes this
...
for i in range(len(abf_files)):
    ...
    header_text = str(cap_file)
    delimiters = ["containing ", r"(\d)+ ", "sweeps"]
    regexPattern = '|'.join(map(re.escape, delimiters))
    num_sweeps_per_file = re.split(regexPattern, header_text)[1]

    # You will get an error if there is only one sweep in the file. Regex is different
    try:
        int(num_sweeps_per_file)
    except ValueError:
        num_sweeps_per_file = 1

    for sw in range(int(num_sweeps_per_file)):
        cap_file_text = file_names_text[i] + f"_sweep{sw + 1}"
        print(cap_file_text)
        sheet["A" + f"{row}"] = cap_file_text
        main(sweep_number=sw)
